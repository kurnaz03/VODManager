...
@router.get("/live/radio/{username}/{password}/{channel_id}.m3u8", tags=["stream"])
...
async def serve_radio_channel(
    ...
    Authenticates the IPTV user, locates the RadioContent's local HLS m3u8
    generated by FFmpeg, rewrites segment URLs to absolute paths.
    """
    user = _auth_iptv_user(db, username, password)

    channel = db.query(RadioContent).filter(RadioContent.id == channel_id).first()
    if channel is None:
    ...
    hls_path = f"{HLS_BASE_DIR}/radio_{channel_id}/stream.m3u8"
    if os.path.isfile(hls_path):
        with open(hls_path, "r") as f:
            m3u8_content = f.read()
        hls_base = f"http://{_server_host()}:{_server_port()}/hls/radio_{channel_id}/"
        rewritten_lines = []
        for line in m3u8_content.splitlines():
            if line.strip() and not line.startswith("#"):
                rewritten_lines.append(hls_base + line.strip())
